...
local function loadProject(projectId, project, name, projectType)
    ...
    if project.pch_auto_generate == nil then
        project.pch_auto_generate = false
    end

    if project.pch_auto_generate then
    ...
    for i, projectType in ipairs(proj.types) do
        ...
        project(proj.alias .. projectNameSuffix(projectType))
            ...
            if proj.pch_enabled and isProjectTypeMainType(projectType) then
                ...
                if proj.pch_auto_generate then
                    local pchContent = "#pragma once\n// THIS FILE IS GENERATED BY THE RYTHE BUILD SYSTEM\n#define RYTHE_PCH\n\n";
